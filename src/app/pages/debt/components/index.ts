
export * from './card-amortization/card-amortization.component';
export * from './card-details/card-details.component';
export * from './search-filter-debt/search-filter-debt.component';  
export * from './table-amortization/table-amortization.component';
export * from './skeleton/skeleton-card-detail-transaction/skeleton-card-detail-transaction.component';
export * from './skeleton/skeleton-debts/skeleton-debts.component';
export * from './debt-search/debt-search.component';
export * from './debt-card/debt-card.component';

export * from './skeleton/skeleton-table-amortization/skeleton-table-amortization.component';
export * from './skeleton/skeleton-card-amortization/skeleton-card-amortization.component';
